...
The files under `upstream/` retain their original Apache-2.0 headers and are redistributed under `LICENSE`. Relative to the pinned commit, Lumen forces Playwright's `msedge` channel and applies six lint cleanups (three placeholder-free f-strings, two unused exception bindings, and one unused import). Lumen's `worker.py` adds a JSON-lines protocol, Edge confinement, cancellation, and explicit approval callbacks without exposing Python process control to the webview.
